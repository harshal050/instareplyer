...
class CampaignService {
    async createCampaign(userId, data) {
        const account = await database_1.InstagramAccountModel.findOne({ _id: data.instagramAccountId, userId });
        if (!account) {
            throw new errors_js_1.NotFoundError('Instagram account not found');
        }
        const campaign = await database_1.CampaignModel.create({
            userId,
    ...
    async getCampaigns(userId) {
        const campaigns = await database_1.CampaignModel.find({ userId }).sort({ createdAt: -1 });
    ...
    async updateCampaign(campaignId, userId, data) {
        ...
        if (data.triggerType !== undefined)
            campaign.triggerType = data.triggerType;
        if (data.posts !== undefined)
            campaign.posts = data.posts;
    ...
    async deleteCampaign(campaignId, userId) {
        const campaign = await database_1.CampaignModel.findOneAndDelete({ _id: campaignId, userId });
        if (!campaign) {
            throw new errors_js_1.NotFoundError('Campaign not found');
        }
    }
    async startCampaign(campaignId, userId) {
        ...
        if (campaign.posts.length === 0) {
            throw new errors_js_1.BadRequestError('Campaign must have at least one Instagram post or reel');
        }
        if (campaign.keywords.length === 0 && campaign.triggerType === 'keyword') {
            throw new errors_js_1.BadRequestError('Campaign must have at least one keyword');
        }
        if (campaign.dmTemplate.messages.length === 0) {
            throw new errors_js_1.BadRequestError('Campaign must have at least one DM message');
        }
    ...
    async processCommentForActiveCampaign(data, sendPrivateReply, userId) {
        const campaigns = await database_1.CampaignModel.find({
            ...(userId ? { userId } : {}),
            status: 'active',
            'posts.postId': data.postId,
        }).select('+processedCommentIds').sort({ createdAt: 1 });
        if (campaigns.length === 0) {
            throw new errors_js_1.NotFoundError('No active campaign found for this post');
        }
        for (const campaign of campaigns) {
            ...
            if (!message) {
                ...
                await campaign.save();
                throw new errors_js_1.BadRequestError('Campaign has no DM message');
            }
            const account = await database_1.InstagramAccountModel.findOne({
            ...
            if (!account) {
                campaign.analytics.dmsFailed += 1;
                await campaign.save();
                throw new errors_js_1.BadRequestError('Connected Instagram account is missing or inactive');
            }
            const accessToken = env_js_1.env.facebook.pageAccessToken || account?.accessToken;
            if (!accessToken) {
                campaign.analytics.dmsFailed += 1;
                await campaign.save();
    ...
    markCommentProcessed(campaign, commentId) {
    ...
    commentMatchesCampaign(text, campaign) {
